...
import com.example.android.popularmoviess1.Utilities.JsonParser;
import com.example.android.popularmoviess1.Utilities.MovieDatabase;
...
public class MainActivity extends AppCompatActivity implements MovieAdapter.MovieAdapterOnClickerHandler {
    ...
    @Override
    public void ItemClicked(MovieEntity movieEntity) {
        Context context = this;
        Class destinationActivity = MovieDetailActivity.class;

        Intent startMovieDetailIntent = new Intent(context, destinationActivity);

        startMovieDetailIntent.putExtra(IntentItems.MOVIE_ENTITY, movieEntity);

        startActivity(startMovieDetailIntent);
    }

    public class FetchMovieTask extends AsyncTask<String, Void, List<MovieEntity>> {
        @Override
        ...
        protected List<MovieEntity> doInBackground(String... params) {
            ...
            String json = MovieDatabase.getMovies(type);
            List<MovieEntity> movieEntities = JsonParser.parseJson(json);
